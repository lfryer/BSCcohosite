...
function storeAddress() {
  ...
  if( !isset($_GET['address']) ){
    ...
    if(!preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*$/i", $address)) {
      ...
      $query_str = "INSERT INTO mailinglist (email, name) VALUES ('" . $address . "', '" . $name . "')";
      $result = mysql_query("INSERT INTO mailinglist (email, name) VALUES ('" . $address . "', '" . $name . "')");
      if(mysql_error()){
        $message = "<strong>Error</strong>: There was an error storing your email address. Here's the query string: " . $query_str . " Here is the error: " . mysql_error();
